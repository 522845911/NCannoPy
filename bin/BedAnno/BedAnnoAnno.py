...
class BedAnnoAnno(object):
	...
	Args    : ofst is total offset to the start(left) of current annoblk,
			  tid is the transcript id for the detail entry
			  tidDetail is the current annoblk detail
	...
	def cal_hgvs_pos(annoEnt, tid=None, tidDetail=None, offset=None, LR=None, noassign=None):
		'''
		NM_001012957.1|DISC1|27185|+|PROM|167|.|-7500|-1|-53-u7500|-53-u1|7500||N|0;
		NM_020897.2|HCN3|57657|+|3U1E|205|EX8E|2490|3874|*1|*1385|1385||Y|244
		NM_033486.1|CDK11B|984|-|?|annotation-fail|EX3|603|339|492|228|858|E,1585964,1586822,GTGACAGCCC|N|0;
		'''
		...
		if lr:  # only one time assignment for one var, offset for left
			...
			if lofst < 0:
				...
				# 6. intron
					...
					if str(gpSO) == 'abnormal-intron':
						...
						if int(lofst) >= half_length:  # drop into latter part
					...
					raise Exception("Error: what's this? " + rtidDetail - ['exin'] + "\n")
			...
			if int(lofst) > int(rtidDetail['wlen']):
				...
				if noassign is None:
					raise Exception("Error: right preceed the block [" + str(lofst) + " < 0]")
				else:
					return 0
			else:  # 0 <= $lofst <= $$rtidDetail{wlen}
				...
				# 4. check if a promoter
					...
					if m:
						if strd:
							#NM_152486.2|SAMD11|148398|+|PROM|167|.|-7500|-1|-80-u7500|-80-u1|7500||Y|0
							cDot = "{}{}{}".format(m.group(1), '-u', (int(m.group(2)) - real_ofst))
						else:
							#NM_001005277.1|OR4F16|81399|-|PROM|167|.|-1|-7500|1-u1|1-u7500|7500||Y|0
							cDot = "{}{}{}".format(m.group(1), '-u', (int(m.group(2)) + real_ofst))
				...
				# 6. intron
					...
					if gpSO == 'abnormal-intron':
						...
						if int(real_ofst) >= half_length:  # drop into latter part
							m = re.match(r'(\d+\+?)(\-?\d+)$', rtidDetail['nsto'])
							if m:
								nDot = "{}{}".format(m.group(1), (int(m.group(2)) - rofst)) if strd else "{}{}".format(
									m.group(1), (int(m.group(2)) + rofst))
					...
					if not hasattr(annoEnt,"trInfo") or tid not in annoEnt.trInfo or "trAlt" not in annoEnt.trInfo[tid]:
						...
						if annoEnt.cal_hgvs_pos(tid=tid, tidDetail=rtidDetail, offset=total_left_ofst,LR=1):  # LR=1 left mode
							if re.match(r'PROM', rtidDetail['blka']):
								...
								if len(tmp_trInfo['trRefComp']['P0'])==0:
									tmp_trInfo['trRefComp']['P0'].insert(0, 0)
								elif len(tmp_trInfo['trRefComp']['P0'])>=1:
									tmp_trInfo['trRefComp']['P0'][0]=0
								...
								if len(tmp_trInfo['trRefComp'][rtidDetail['exin']])==0:
									tmp_trInfo['trRefComp'][rtidDetail['exin']].insert(0,0)
								elif len(tmp_trInfo['trRefComp'][rtidDetail['exin']])>=1:
									tmp_trInfo['trRefComp'][rtidDetail['exin']][0]=0
							elif not strd:
						...
						if re.match(r'EX', tmp_exin):
							if (tid not in tidExblk) or (tmp_exin not in tidExblk[tid]) or ("{}{}{}".format(rtidDetail['nsta'], ',', rtidDetail['nsto']) not in tidExblk[tid][tmp_exin]):
								...
								if (not re.match(r'\d+$', str(cmpStart))) or (not re.match(r'\d+$', str(cmpEnd))):
									raise Exception("{}{}{}{}{}".format("Error: unknown bug cmpStart [",cmpStart , "], cmpEnd [", cmpEnd, "]"))
								...
								if m and strd and re.search(r'E$', str(tmp_exin)):
									...
									if len(trinfoEnt['trRefComp']['Z999'])==0:
										trinfoEnt['trRefComp']['Z999'].insert(0 , int(unify_rl) - int(m.group(1)))
									elif len(trinfoEnt['trRefComp']['Z999'])>=1:
										trinfoEnt['trRefComp']['Z999'][0]=int(unify_rl) - int(m.group(1))
									if len(trinfoEnt['trRefComp']['Z999'])<2:
										trinfoEnt['trRefComp']['Z999'].insert(1 ,int(unify_rl))
									elif len(trinfoEnt['trRefComp']['Z999'])>=2:
										trinfoEnt['trRefComp']['Z999'][1]=int(unify_rl)
							...
							if int(rannodb[k]['sto']) > (int(unify_p) + int(unify_rl)):
								...
								less = rannodb[k]['sto']
							if len(trinfoEnt['trRefComp'][tmp_exin])==1:
								if len(trinfoEnt['trRefComp'][tmp_exin])<2:
									trinfoEnt['trRefComp'][tmp_exin].insert(1,int(less) - int(unify_p))
								elif len(trinfoEnt['trRefComp'][tmp_exin])>=2:
									trinfoEnt['trRefComp'][tmp_exin][1]=int(less) - int(unify_p)
							elif len(trinfoEnt['trRefComp'][tmp_exin])>=2:
								trinfoEnt['trRefComp'][tmp_exin][1]=int(less) - int(unify_p)
