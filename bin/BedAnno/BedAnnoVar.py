...
class BedAnnoVar(object):
	...
	def __init__(self, *args):  # chr, start, end, ref, alt
		...
		if (len(args) == 1 and isinstance(args[0], (dict))):
			...
			if (("chr" not in var_dict) or ("begin" not in var_dict) or (
				...
				raise Exception("Error: unavailable object. need keys: " + "chr, start, alt, ref specified.")
			...
			if ("variantSequence" in var_dict and var_dict["variantSequence"] is not None):
				...
				alt = ""
			if (re.search("^null$", str(alt), flags=re.IGNORECASE)):
				alt = ""
			...
			if (len(args) < 4):
				raise Exception("Error: not enough args, need at least 4 args.")
			if (len(args) == 4):
		...
		if end is None or (not re.match("^\d+$", str(end))):  # from VCF v4.1 1-based start
			...
			if (ref != alt and len(ref) >=1 and len(alt)>=1 and ref[0] == alt[0]) or (ref == alt  and len(ref) >1 and len(alt)>1 ):
			#if (ref != alt or  len(ref) >1) and ref[0] == alt[0]:
				ref = ref[1:]
				alt = alt[1:]
			else:
	...
	def normalise_seq(seq):
		seq, _ = re.subn(r"\s+", "", str(seq))
		if seq == '.':
			seq = ""
		seq = seq.upper()
		m = re.search(r"[^ACGTN]", seq)
		if m:
			raise Exception("Error: unrecognized pattern exists,no multiple alts please. [" + seq + "]")
		return seq
	...
	def check_insrep(larger, smaller, repeat, cn):
		'''
		检测larger 序列是否是由smaller序列中插入cn个repeat序列构成，如果是，则返回1，否则，返回0
		'''
	...
	def count_content(s):
		'''
		s为蛋白序列或碱基序列，因为碱基序列的种类要远远小于氨基酸序列的种类
		返回一个列表，其第一个元素为氨基酸的总个数，其余元素为每种氨基酸的计数
		'''
		s = s.upper()
		l = len(s)
		...
		for char in s:
			if (char not in BedAnno.BedAnno.AAnumber):
				raise Exception("unknown base [" + char + "]")
	...
	def check_div(s, rc):
		'''
		s为氨基酸序列,rc为另外一个经过count_content处理返回的结果，从氨基酸序列的长度和各种氨基酸的数量上判断，rc是否为rcs的整数倍。
		'''
	...
	def getUnifiedVar(self, strd):
		'''
		返回标准的pos,ref,alt,ref_len,alt_len
		'''
		consPos = self.pos
		consRef = self.ref
		consAlt = self.alt
	...
	def get_gHGVS(self):
		...
		if (imp == 'snv'):
			...
			raise Exception("Can not recognize type " + str(imp) + ".")
		self.gHGVS = gHGVS
	
	@LogExc
	def parse_complex(self):
		'''
		对于一些ref和alt，如果左边和右边对应相等的碱基个数小于ref或alt中最短的，就不区分+,-链。如果超过最短者，这区分+,-链。然后处理较长的一条链是否是由较短的一条链插入可能的教程的重复序列组成的。在self中添加有关重复的信息。单元重复序列应为重复序列中最长的。
